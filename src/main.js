...
import { runtime } from './face/face-runtime.js';
import {
  initDeform, buildFaceMask, faceDeformNode, writeFaceExpr, updateFaceUniforms,
  refreshFaceMaskOverlays, downloadFaceMasks, faceMaskOverlays
} from './face/deform.js';
import {
...
async function init() {
  ...
  // T31: inject the app singletons into the extracted face modules (src/face/*).
  const faceCtx = { scene, camera, controls, renderer, state, crowdBatches, statusEl };
  initDeform(faceCtx);
  initEditor(faceCtx);
...
function clearWalkers() {
  ...
  for (const batch of crowdBatches) {
    ...
    if (batch.faceMaskOverlay) { // T27: drop the debug vertex cloud with its batch
      ...
      faceMaskOverlays.delete(batch.faceMaskOverlay);
    }
  }
...
// latestFaceExpr + debugFaceOverride now live in face-runtime.js (shared with the face modules).
...
function updateCanonical(frame) {
  ...
  if (raw) {
    ...
    if (frame.faceBlendshapes) {
      runtime.latestFaceExpr = blendshapesToExpr(frame.faceBlendshapes);
      faceStats.neutral = 'blendshapes';
    } else {
      runtime.latestFaceExpr = faceExpr.update(latestCanonical, faceDt, { thresh: state.poseKptThresh });
      faceStats.neutral = faceExpr.neutral ? 'set' : 'capturing…';
    }
    Object.assign(faceStats, runtime.latestFaceExpr);
...
if (import.meta.env.DEV) {
  window.__vj = {
    ...
    faceExpr,
    get face() { return runtime.latestFaceExpr; },
    camera,
    controls,
    refreshFaceMaskOverlays,
    setFace(o) { runtime.debugFaceOverride = o; },
    clearFace() { runtime.debugFaceOverride = null; },
    faceEditor,
    setFaceEditMode,
    faceEditReseed,
    faceEditPaint,
