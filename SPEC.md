...
- V29: face deform lives in `positionNode` compute path, ⊥ `morphTargetInfluences` (bypasses the custom compute draw, V1/V2). Per-instance expr = instanced attribute (same per-instance machinery as `boneMatrices`, V5). Jaw = hinge-rotate verts about a head-local axis, ⊥ linear translate (linear looks wrong).
...
T26|.|**face phase 1 — extract**: `FaceExpression` from landmarks 23-90 → 7 scalars (MAR jawOpen, corner-width smile, pucker, EAR blinkL/R, brow-eye browL/R); neutral calib + OneEuro; Inspector live plot. Also fix `MIRROR_INDEX` face gap (add 23-90 L/R pairs)|V26,V30,I.faceExpr
