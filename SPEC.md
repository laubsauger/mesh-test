...
B9|2026-07-01|retarget jumpy + "doesn't follow" for neck/head/torso. Causes: (1) binary conf gate `conf<thresh→return` froze a bone then POPPED on reacquire; (2) monocular z mixed into every aim tilted short-baseline bones (neck) + long torso lean from noise; (3) torso = ONE rigid Hips bone (no spine, B3) → real torso flex had nowhere to go; (4) shoulders not mapped at all|(1) soft gate `confGate` smoothstep → low conf eases bone to REST (no pop); (2) z-damp HIPS_DEPTH 0.3→0.12, neck NECK_DEPTH 0.85→0.3; (3) `_aimTorso` distributes ONE world delta Q across SPINE_CHAIN (hips.15/spine.28/spine01.28/spine02.29 cumulative→full at top) as `Q^f∘bindWorld` (world-delta = bone-rest-dir-agnostic, dodges B3 -y flip); pelvis barely tilts (legs grounded), curl lives in spine; (4) `_aimClavicles` shrug/protraction at 0.6 gain
